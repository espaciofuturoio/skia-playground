...
export const CombinedScaleSVGNative = ({
  greyLeftGrid = [],
  greyRightGrid = [],
  redGrid = [],
  showGreyScale = true,
    ...
    <View>
      <View style={{ width: canvasSizeX, height: canvasSizeY, ...debug ? { borderWidth: 1, borderColor: "red" } : {} }}>
        <Svg
          ...
          preserveAspectRatio="xMidYMid meet"
        >
          <G origin={`${canvasSizeX / 2}, ${canvasSizeY / 4}`}>
            {/* Grey Scale */}
            {showGreyScale && (
              <>
                <SvgXml
                ...
                <G origin={`${greyXOriginScale}, ${Math.round(greyYOriginScale - 35)}`}>
            ...
            {showRedScale && (
              <>
                ...
                <G origin={`${redXOriginScale}, ${Math.round(redYOriginScale - 35)}`}>
